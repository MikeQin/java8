 ...
 * count() reduce() forEach() forEachOrdered() min() max() anyMatch() allMatch()
...
public class ReduceFunction {
	...
	public void parallelProcessing() {
		...
		int totalAges = persons.parallelStream()
		    .reduce(0, (sub, p) -> (sub + p.getAge()), Integer::sum);
		System.out.println("Total ages in parallel: " + totalAges);

		totalAges = persons.stream()
		    .parallel()
		    .reduce(0, (sub, p) -> (sub + p.getAge()), (sum, thread) -> (sum + thread));
		...
		Map<String, List<Person>> result = persons.stream()
		    ...
		    .reduce(initMap, (resultMap, person) -> {
			    if (resultMap.get(person.getOccupation()) == null) {
				    ...
				    resultMap.get(person.getOccupation())
				        .add(person);
			    }
			    return resultMap;
		    }, (x, y) -> {
